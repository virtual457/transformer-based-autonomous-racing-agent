...
Each action channel gets its own reward signal:
    r[0] (steer):    gap + yaw + smoothness_steer
    r[1] (throttle): speed_throttle + progress + gap + smoothness_throttle
...
Speed decomposition (pure intent-based):
    ...
    Reward is proportional to action magnitude:
      half throttle below target → +0.5
      full throttle below target → +1.0
...
def _speed_rewards_per_channel(
    ...
    if speed_ms < target:
        r_throttle = throttle_action       # +1 at full throttle, -1 at off
        r_brake    = -brake_action         # +1 at brake off (-1), -1 at full brake (+1)
    else:
...
def compute_vector_reward_detailed(
    ...
    tc = {
        ...
        "progress": W_THROTTLE["progress"] * r_progress,
...
def compute_vector_reward(
    ...
    r_throttle = (
        ...
        + W_THROTTLE["progress"] * r_progress

"""
network.py — Neural network architectures for Transformer SAC (Variant D).
...
class PolicyHead(nn.Module):
    ...
    def forward(self, embedding: torch.Tensor):
        ...
        # log-prob corrected for tanh squash — exact copy from gym/sac/network.py:
...
class TwinQHead(nn.Module):
    ...
    action is (B, action_dim).  Each network outputs a scalar Q-value.
    ...
    def __init__(
        ...
        self.q1_net = _make_mlp(in_dim, 1, hidden_units)
        self.q2_net = _make_mlp(in_dim, 1, hidden_units)

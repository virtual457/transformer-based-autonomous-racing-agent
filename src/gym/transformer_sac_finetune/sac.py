...
class TransformerSAC:
    ...
    def update_from_batch(self, batch: dict) -> dict:
        ...
        with torch.no_grad():
            ...
            next_action, next_log_prob, _ = self.policy_head(next_emb_policy)  # next_log_prob: (B, action_dim)
            ...
            # Per-action entropy term: sum(alpha_d * log_prob_d) over action dims → (B, 1)
            next_entropy_term = (alpha.unsqueeze(0) * next_log_prob).sum(dim=-1, keepdim=True)
        ...
        new_action, log_prob, _ = self.policy_head(obs_emb_policy)  # log_prob: (B, action_dim)
        ...
        # Per-action entropy term summed to (B, 1) for policy loss.
        entropy_term = (alpha.unsqueeze(0) * log_prob).sum(dim=-1, keepdim=True)
        policy_loss = (entropy_term - min_q_new).mean()
        ...
        # ------------------------------------------------------------------
        # Per-action alpha loss: mean over batch first, then per-dim multiply, then mean over dims.
        # log_prob: (B, action_dim) → mean over batch: (action_dim,)
        ...
        # Clamp each per-action log_alpha independently.
        with torch.no_grad():
        ...
        # Logging values
        # ------------------------------------------------------------------
        # Per-action entropy (mean over batch per dim) and mean alpha.
        per_action_entropy = (-log_prob.detach().mean(dim=0).cpu()).tolist()  # [H_steer, H_throttle, H_brake]
        ...
        return {
            ...
            "alpha":              self.alpha.mean().item(),        # mean for logging
            "alpha_per_action":   self.alpha.detach().cpu().tolist(),  # [a_steer, a_throttle, a_brake]
            "entropy":            entropy,

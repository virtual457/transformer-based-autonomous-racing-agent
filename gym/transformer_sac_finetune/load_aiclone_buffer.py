"""
load_aiclone_buffer.py — Seed the finetune DualWindowReplayBuffer with AIClone data.

Identical logic to transformer_sac/load_aiclone_buffer.py but:
  - Imports from transformer_sac_finetune.replay_buffer (isolated copy)
  - Saves to gym/transformer_sac_finetune/checkpoints/

Run to reset the positive buffer back to clean AIClone data:
    .\\AssetoCorsa\\Scripts\\python.exe gym/transformer_sac_finetune/load_aiclone_buffer.py

The buffer is auto-loaded by FinetuneAgent on the next training run.
"""
...
WINDOW_SIZE    = 75
CAPACITY       = 50_000
DEMO_REWARD    = 0.9   # near-perfect demonstration reward
...
def main():
    ...
    for ep_idx in range(E):
        ...
        ep_rewards = np.full(e - s, DEMO_REWARD, dtype=np.float32)
    ...
    logger.info(
        f"Sanity check — batch reward: min={batch['reward'].min():.3f}  "
        f"max={batch['reward'].max():.3f}  "
        f"steer mean={batch['action'][:,0].mean():.3f}"
    )

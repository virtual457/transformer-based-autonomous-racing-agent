...
class EgoServer:
    def __init__(self, config, car, controls, telemetry, track):
        ...
        self.observers = []   # read-only clients; do not affect control path
            ...
            while self.server_socket and self.socket_open:
                ...
                try:
                    ...
                    if data == "connect":
                        ...
                        if addr not in self.observers:
                            self.observers.append(addr)
                            logger.info("[EGO SERV] Observer added: {}".format(addr))
                        self.server_socket.sendto(b"identified", addr)
                    elif data == "unobserve":
                        if addr in self.observers:
                            self.observers.remove(addr)
                            logger.info("[EGO SERV] Observer removed: {}".format(addr))
                    elif data == "reset":
    ...
    def tick(self):
        ...
        # Fan-out to read-only observers — runs independently of current_client
        if self.observers:
            if (self.total_steps % (self.config.sampling_freq // self.config.ego_sampling_freq)) == 0:
                self.car.update(self.track)
                obs_export = self.car.export()
                dead = []
                for obs_addr in self.observers:
                    try:
                        self.server_socket.sendto(obs_export.encode(), obs_addr)
                    except Exception:
                        dead.append(obs_addr)
                for obs_addr in dead:
                    self.observers.remove(obs_addr)
                    logger.info("[EGO SERV] Observer dropped (send failed): {}".format(obs_addr))

...
def plot_episode(
    ...
    # Component traces.
    for comp_key in ["gap", "yaw", "smooth", "crash"]:
    ...
    for comp_key in ["speed", "progress", "gap", "smooth", "crash"]:
    ...
    if metrics:
        ...
        fig.add_hline(y=0, line=dict(color="#555", dash="dash", width=0.6))
        fig.update_layout(**_base("Gap from Racing Line (m)", 300),
                          yaxis=dict(gridcolor=_GRID, title="metres", rangemode="tozero"),
                          xaxis=dict(gridcolor=_GRID, title="frame"))

"""
train.py — Vector Q variant: Decomposed per-action reward with 3-channel Q output.

Key difference from transformer_sac_finetune:
  - Reward is a (3,) vector: [r_steer, r_throttle, r_brake]
  - Q network outputs (B, 3) instead of (B, 1)
  - Per-channel Bellman targets and per-channel policy loss
  - Replay buffer stores (capacity, 3) rewards

Run command:
    .\\AssetoCorsa\\Scripts\\python.exe gym/transformer_sac_vectorq/train.py --manage-ac
"""
...
for _name in ("vectorq_transformer_sac", "transformer_sac_vectorq"):
    _log = logging.getLogger(_name)
    _log.setLevel(logging.INFO)
    _log.propagate = True

logger = logging.getLogger("vectorq_transformer_sac")
...
def main():
    ...
    parser.add_argument(
        "--no-ai-drive", action="store_true",
        help="Disable AI autopilot start randomization (only relevant with --manage-ac).",
    )
    ...
    work_dir    = os.path.join(_THIS_DIR, '..', '..', 'outputs', 'transformer_sac_vectorq')
    ...
    # ── 5. TransformerSAC ─────────────────────────────────────────────────────
    from transformer_sac_vectorq.sac import TransformerSAC
    ...
    # ── 7. Agent ──────────────────────────────────────────────────────────────
    from transformer_sac_vectorq.agent import FinetuneAgent

...
class AssettoCorsaEnv(Env, gym_utils.EzPickle):
    ...
    def get_obs(self, state, history=None):
        ...
        # ISSUE-011 Fix 1: when history is shorter than PAST_ACTIONS_WINDOW,
        # fill missing slots by repeating the current state's own action instead
        # of using zeros.  The BC model never saw all-zeros history at high speed
        # during training (training data starts mid-lap with populated history),
        # so zeros cause an OOD left-steer bias.  Warm-starting with the current
        ...
        if len(history) < PAST_ACTIONS_WINDOW:
            ...
            current_controls_steer_prev = np.array(
            ...
            if len(history) > 0:
                actions_diff = np.array([
                    (state['steerAngle']  - history[-1]['steerAngle'])  / _steer_scale,
        ...
        obs = np.hstack([obs, _cur_steer, _cur_pedal, _cur_brake])

        #
        if self.add_previous_obs_to_state:
            if len(history) < PAST_ACTIONS_WINDOW:
                # Warm-start: fill missing slots with current state's basic obs
                # instead of zeros, consistent with preprocess_bc.py (ISSUE-011 Fix 1).
                _n_missing = PAST_ACTIONS_WINDOW - len(history)
                _cur_basic = get_basic_obs(state)
                prev_obs = np.hstack(
                    [_cur_basic] * _n_missing
                    + [get_basic_obs(history[i]) for i in range(len(history))]
                )
            else:
                prev_obs = []
    ...
    def set_track(self, track_name):
        ...
        grid_file = self.track_config.get("track_grid_file")
        self.track_grid_file = os.path.join(self.tracks_path, grid_file) if grid_file else None

...
class FinetuneAgent:
    ...
    def collect_phase(self, phase_num: int) -> dict:
        ...
        for ep in range(n_episodes):
            ...
            # Randomize start position: Ctrl+C → AI drives 25s → Ctrl+C → env.reset()
            ...
            ep_vec_breakdowns: list = []  # per-channel component dicts from compute_vector_reward_detailed
            ...
            for _ in range(_warmup_steps):
                ...
                # Compute vector reward for warmup frames.
            ...
            while not done:
                ...
                self.env._read_latest_state()
                # Compute per-action vector reward from components.
            ...
            if ep_steps > 0:
                try:
                    from transformer_sac_vectorq.episode_logger import plot_episode
    ...
    def train_phase(self, phase_num: int, steps_collected: int = 0) -> dict:
        ...
        q_loss_per_ch_buf      = []   # list of [q_steer, q_throttle, q_brake]
        ...
        for step in range(1, n_steps + 1):
            ...
            if step % log_interval == 0:
                ...
                qpc = np.mean(q_loss_per_ch_buf[recent],      axis=0)   # (3,)
                logger.info(
                    f"  step={step}/{n_steps}  "
